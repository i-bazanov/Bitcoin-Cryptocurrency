...
public class UTXOPool {
    ...
    /** Creates a new empty scroogecoin.main.UTXOPool */
    public UTXOPool() {
        H = new HashMap<UTXO, Transaction.Output>();
    }

    /** Creates a new scroogecoin.main.UTXOPool that is a copy of {@code uPool} */
    public UTXOPool(UTXOPool uPool) {
        H = new HashMap<UTXO, Transaction.Output>(uPool.H);
    }

    /** Adds a mapping from scroogecoin.main.UTXO {@code utxo} to transaction output @code{txOut} to the pool */
    public void addUTXO(UTXO utxo, Transaction.Output txOut) {
        H.put(utxo, txOut);
    }

    /** Removes the scroogecoin.main.UTXO {@code utxo} from the pool */
    public void removeUTXO(UTXO utxo) {
        H.remove(utxo);
    }

    /**
     * @return the transaction output corresponding to scroogecoin.main.UTXO {@code utxo}, or null if {@code utxo} is
     *         not in the pool.
     */
    ...
    /** @return true if scroogecoin.main.UTXO {@code utxo} is in the pool and false otherwise */
